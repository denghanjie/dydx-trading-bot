...
def manage_trade_exits(client):
    """
        Managing exiting open positions
    ...
    try:
        ...
        print("Error opening bot_agents.json")
        return "complete"
    ...
    all_exec_pos = exchange_pos.data["positions"]
    markets_live = []
    for p in all_exec_pos:
    ...
    for position in open_positions_dict:
        ...
        # Extract position matching information from file - markert 1
        position_market_m1 = position["market_1"]
        position_size_m1 = position["order_m1_size"]
        position_side_m1 = position["order_m1_side"]
    
        # Extract position matching information from file - markert 2
        ...
        check_m1 = str(position_market_m1) == str(order_market_m1) and position_side_m1 == order_side_m1 and float(position_size_m1) == float(order_size_m1)
        check_m2 = str(position_market_m2) == str(order_market_m2) and position_side_m2 == order_side_m2 and float(position_size_m2) == float(order_size_m2)
        ...
        if not check_m1 or not check_m2 or not check_live:
            ...
            print(f"check_m1: {check_m1}, position_market: {position_market_m1 == order_market_m1}, position_side: {position_side_m1 == order_side_m1}, position_size: {position_size_m1 == order_size_m1}")
            print(f"{position_market_m1}, {order_market_m1}, {position_size_m1}, {order_size_m1}")
            print(f"check_m2: {check_m2}, position_market: {position_market_m2 == order_market_m2}, position_side: {position_side_m2 == order_side_m2}, position_size: {position_size_m2 == order_size_m2}")
            print(f"{position_market_m2}, {order_market_m2}, {position_size_m2}, {order_size_m2}")
            print(f"check_live: {check_live}, position_market_m1 in markets_live? {position_market_m1 in markets_live}, position_market_m2 in markets_live? {position_market_m2 in markets_live}")
            continue
        ...
        if is_close:
            ...
            ticke_size_m1 = markets["markets"][position_market_m1]["tickSize"]
            ticke_size_m2 = markets["markets"][position_market_m2]["tickSize"]
            accept_price_m1 = format_number(accept_price_m1, ticke_size_m1)
            accept_price_m2 = format_number(accept_price_m2, ticke_size_m2)
